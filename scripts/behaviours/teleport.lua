Teleport = Class(BehaviourNode, function(self, inst, hunterparams, see_dist, safe_dist, fn, runhome, fix_overhang, walk_instead, safe_point_fn)
    BehaviourNode._ctor(self, "Teleport")
...
function Teleport:__tostring()
    return string.format("Teleport %f from: %s", self.safe_dist, tostring(self.hunter))
end

-- function Teleport:GetRunAngle(pt, hp, sp)
...
function Teleport:Visit()
    if self.status == READY then
        self.hunter = FindEntity(self.inst, self.see_dist, self.hunterfn, self.huntertags, self.hunternotags, self.hunteroneoftags)

        if self.hunter ~= nil and self.shouldteleportfn ~= nil and not self.shouldteleportfn(self.hunter, self.inst) then
            self.hunter = nil
        end
    ...
    if self.status == RUNNING then
        if self.hunter == nil or not self.hunter.entity:IsValid() then
            ...
            local pt = self.inst:CalculateLandPoint(pt)
            if pt ~= nil then
                self.inst:Pushevent(math.random(1, 2) == 1 and "use_portal_jumpin" or "use_pocketwatch_portal", pt)
            else
                self.status = FAILED
                self.inst.components.locomotor:Stop()
            end
